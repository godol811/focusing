...
struct InAppPurchaseView: View {
    ...
    var conditions = """
    - 사용자가 앱서비스, 데이터 캐시 등을 삭제하게 되면 결제한 유료 상품에 대한 데이터가 사라지며 이에 대한
    복원 및 환불이 불가능하니 주의하시기 바랍니다.
    
    - 결제는 인앱결제(앱스토어, 플레이스토어)를 통해 이루어지며 결제 및 환불 과정에 대한 정보는 당사에서
    관리하는 것이 아닌 스토어에서 보관 및 처리되며 각사의 서비스 약관에 따릅니다.
    
    - 각 스토어의 약관에 따라 구매자가 결제한 스토어에 직접 환불을 요청해야할 수 있으며, 각 스토어의 환불 정
    책에 따라 환불이 거부될 수 있습니다. 또한 회원이 구매한 상품을 이미 사용한 경우에는 환불이 불가능합니다.
...
//                        .padding(.bottom, 43)
                ...
                .frame(width: UIScreen.main.bounds.width - 52 , height: UIScreen.main.bounds.height - 100, alignment: .leading)
